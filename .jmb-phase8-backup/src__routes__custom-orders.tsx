...
function CustomOrdersPage() {
  ...
  async function createGuest(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      toast.success("Custom request created", { description: result.emailSent === false ? "Your request is saved, but the email could not be sent. Bookmark the private chat page that opens next." : "A private chat link has also been emailed to you." });
      window.location.assign(`/guest/custom/${result.request.id}?token=${encodeURIComponent(result.guestToken)}`);
