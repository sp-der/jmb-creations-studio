...
import { CATEGORIES, PRODUCTS, SOCIAL } from "@/data/catalog";
    ...
    <StoreLayout>
      <section className="relative overflow-hidden bg-gradient-hero">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-10 px-4 py-16 sm:px-6 md:py-24 lg:grid-cols-2">
          ...
          <div className="relative mx-auto w-full max-w-md">
            <div className="relative rounded-[2.5rem] bg-card p-4 shadow-lift"><img src={logo.url} alt="JMB 2 Creations logo" className="w-full rounded-[2rem]" width={640} height={640} /></div>
            {HERO_FLOAT_IMAGES[0] && <img src={HERO_FLOAT_IMAGES[0]} alt="JMB creation" className="absolute -left-8 -top-8 hidden w-28 rotate-[-8deg] animate-float-slow rounded-[1.5rem] shadow-soft sm:block" />}
            {HERO_FLOAT_IMAGES[1] && <img src={HERO_FLOAT_IMAGES[1]} alt="JMB creation" className="absolute -bottom-8 -left-5 hidden w-24 rotate-6 animate-float-mid rounded-[1.4rem] shadow-soft sm:block" />}
            {HERO_FLOAT_IMAGES[2] && <img src={HERO_FLOAT_IMAGES[2]} alt="JMB creation" className="absolute -right-8 bottom-10 hidden w-28 rotate-[10deg] animate-float-slow rounded-[1.5rem] shadow-soft sm:block" />}
          </div>
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-4 py-16 sm:px-6">
        ...
        <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
          {CATEGORIES.map((category) => <Link key={category.slug} to="/category/$slug" params={{ slug: category.slug }} className="card-lift group overflow-hidden rounded-[1.75rem] border border-border bg-card p-3 shadow-soft"><img src={category.image} alt={category.name} className="aspect-[4/5] w-full rounded-2xl object-cover" loading="lazy" /><div className="px-2 py-3"><h3 className="text-base font-bold">{category.name}</h3><p className="mt-1 text-xs text-muted-foreground">{category.count} designs available</p></div></Link>)}
        </div>
      </section>

      <section className="bg-secondary/30 py-16"><div className="mx-auto max-w-7xl px-4 sm:px-6"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.22em] text-primary">Shop</p><h2 className="mt-2 text-3xl font-bold sm:text-4xl">Product collections</h2></div><Button variant="soft" asChild><Link to="/shop">See all products</Link></Button></div><div className="mt-8 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">{PRODUCTS.map((product) => <ProductCard key={product.id} product={product} />)}</div></div></section>
