...
function ProductDetailPage() {
  ...
  const currentAlt = selectedReady?.name ?? displayDesignName(selected);
  ...
  const customHref = `/custom-orders?product=${encodeURIComponent(product.name)}${selectedReady ? `&design=${encodeURIComponent(selectedReady.name)}` : selected ? `&design=${encodeURIComponent(displayDesignName(selected))}` : ""}`;
            ...
            <div className="mt-4 grid grid-cols-4 gap-3 sm:grid-cols-6">
              {designs.map((item, index) => (
                <button key={item.id} type="button" onClick={() => { setSelectedReadyId(null); setSelectedIndex(index); }} className={`overflow-hidden rounded-xl border p-1 ${!selectedReady && selectedIndex === index ? "border-primary ring-2 ring-primary/15" : "border-border"}`}>
                  <img src={item.image} alt={displayDesignName(item)} className="aspect-[4/5] w-full rounded-lg object-cover" loading="lazy" />
                </button>
              ))}
            </div>
          </section>
