...
import { LayoutDashboard, Menu, Search, ShoppingBag, UserRound, X } from "lucide-react";
...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border/70 bg-background/85 backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl items-center gap-2 px-4 py-1 sm:px-6">
        <Link
          to="/"
        ...
        <nav aria-label="Main" className="ml-auto hidden items-center gap-0.5 lg:flex">
          {NAV_LINKS.map((link) => (
            <Link
              ...
              className="rounded-full px-2.5 py-2 text-[13px] font-semibold text-foreground/80 transition-colors hover:bg-secondary/50 hover:text-foreground xl:px-3 xl:text-sm"
            >
              {link.label}
            </Link>
          ))}
        ...
        <div className="ml-auto flex items-center gap-0.5 lg:ml-2">
          ...
          <Button variant="ghost" size="icon" asChild aria-label="Demo customer portal">
            <Link to="/account">
              <UserRound />
            </Link>
          </Button>
      ...
      {searchOpen && (
        <div className="border-t border-border/70 bg-card/90 px-4 py-3 sm:px-6">
          <form className="mx-auto flex max-w-3xl items-center gap-2" onSubmit={(event) => event.preventDefault()}>
            <label htmlFor="nav-search" className="sr-only">Search creations</label>
            <Input id="nav-search" placeholder="Search charms, toppers, keychains…" className="h-11 rounded-full" />
            <Button type="submit" variant="hero" asChild>
              <Link to="/shop" onClick={() => setSearchOpen(false)}>Search</Link>
            </Button>
