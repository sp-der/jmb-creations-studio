...
function PaymentPage() {
  ...
  async function load() {
    ...
    try {
      ...
      if (currentSession?.access_token) {
        ...
        setGuestToken(token);
        if (token) nextOrder = await fetchGuestOrder(orderId, token);
      }
      setOrder(nextOrder);
  ...
  if (!order) return <main className="grid min-h-[70vh] place-items-center bg-[oklch(0.985_0.01_320)] px-4"><section className="max-w-lg text-center"><ShieldCheck className="mx-auto size-9 text-primary" /><h1 className="mt-4 font-display text-3xl font-bold">We could not open this payment page.</h1><p className="mt-3 text-sm text-muted-foreground">Guest orders need the same browser session or private order link. Signed-in customers can return from their account.</p><Button className="mt-6" variant="hero" asChild><Link to="/account">Customer Account</Link></Button></section></main>;
