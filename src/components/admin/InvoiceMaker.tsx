      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <section className="rounded-[1.75rem] border border-border bg-card p-5 shadow-soft sm:p-6">
          ...
          <div className="mt-6 space-y-4">
            ...
            <div className="flex flex-wrap gap-3"><Button variant="hero" onClick={() => void save()} disabled={busy}><Save /> {busy ? "Saving..." : "Save Invoice & Download PDF"}</Button><Button variant="soft" onClick={() => void downloadPdf()} disabled={!savedInvoice || busy}><Download /> Download PDF</Button><Button variant="soft" onClick={() => window.print()} disabled={busy}><Printer /> Print</Button><Button variant="soft" onClick={() => void emailInvoice()} disabled={!savedInvoice || busy}><Mail /> Email Invoice</Button></div>
          </div>
        </section>

        <section id="jmb-invoice-preview" className="overflow-hidden rounded-[1.75rem] border border-border bg-white shadow-soft">
