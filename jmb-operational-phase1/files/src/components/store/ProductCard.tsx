        ...
        <div className="absolute left-5 top-5 flex flex-col gap-1.5">
          {product.customizable && (
            <Badge className="rounded-full bg-card px-3 text-foreground shadow-soft">
              <Sparkles className="mr-1 size-3" aria-hidden /> Customizable
            </Badge>
          )}
        </div>
      </Link>
      <div className="flex flex-1 flex-col px-5 pb-5">
        <p className="text-xs font-bold uppercase tracking-[0.14em] text-mauve">{product.designs.length} designs available</p>
        <h3 className="mt-1 text-lg font-bold leading-snug">
          <Link to="/product/$slug" params={{ slug: product.slug }} className="hover:text-primary">{product.name}</Link>
        </h3>
        <p className="mt-2 line-clamp-2 text-sm text-muted-foreground">{product.description}</p>
        <Button variant="hero" className="mt-5 w-full" asChild>
          <Link to="/product/$slug" params={{ slug: product.slug }}>
            View Collection <ArrowRight aria-hidden />
          </Link>
        </Button>
      </div>
    </article>
