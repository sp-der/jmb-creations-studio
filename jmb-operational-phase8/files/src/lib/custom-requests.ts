...
export async function fetchGuestWorkspace(requestId: string, token: string) {
  const url = getSupabaseUrl();
  if (!url) throw new Error("Supabase is not configured.");
  const response = await fetch(`${url}/functions/v1/jmb-guest-custom-access`, {
    method: "POST",
    headers: publicHeaders(),
    body: JSON.stringify({ action: "read", requestId, token }),
  });
  if (!response.ok) throw new Error(await parseError(response));
  return await response.json() as { request: CustomRequest; messages: CustomMessage[] };
}

export async function fetchGuestRequest(requestId: string, token: string) {
  return (await fetchGuestWorkspace(requestId, token)).request;
}

export async function fetchGuestMessages(requestId: string, token: string) {
  return (await fetchGuestWorkspace(requestId, token)).messages;
}
...
export async function sendGuestMessage(requestId: string, token: string, body: string) {
  ...
  const response = await fetch(`${url}/functions/v1/jmb-guest-custom-access`, {
    method: "POST",
    headers: publicHeaders(),
    body: JSON.stringify({ action: "send", requestId, token, body }),
  });
  if (!response.ok) throw new Error(await parseError(response));
  const data = await response.json() as { message: CustomMessage };
  return data.message;
}
