...
function ProductDetailPage() {
  ...
  useEffect(() => {
    if (!product) return;
    let cancelled = false;
    fetchLiveCatalogItems(product.slug).then((items) => {
      if (!cancelled) setReadyItems(items);
    }).catch(() => { if (!cancelled) setReadyItems([]); });
  ...
  const currentImage = selectedReady?.image_url ?? selected?.image ?? product.mainImage;
  const currentAlt = selectedReady?.name ?? selected?.name ?? product.name;
  ...
  const addReadyToCart = () => {
    ...
    toast.success("Added to cart", { description: `${quantity} × ${selectedReady.name}` });
  };

  const customHref = `/custom-orders?product=${encodeURIComponent(product.name)}${selectedReady ? `&design=${encodeURIComponent(selectedReady.name)}` : selected ? `&design=${encodeURIComponent(selected.name)}` : ""}`;
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        <div className="mt-7 grid items-start gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(380px,0.82fr)]">
          <section>
            <div className="relative overflow-hidden rounded-[2rem] border border-border bg-card p-4 shadow-lift">
              <img src={currentImage} alt={currentAlt} className="aspect-[4/5] w-full rounded-[1.5rem] object-cover" />
              {designs.length > 1 && !selectedReady && <>
            ...
            <div className="mt-4 grid grid-cols-4 gap-3 sm:grid-cols-6">
              {designs.map((item, index) => (
                <button key={item.id} type="button" onClick={() => { setSelectedReadyId(null); setSelectedIndex(index); }} className={`overflow-hidden rounded-xl border p-1 ${!selectedReady && selectedIndex === index ? "border-primary ring-2 ring-primary/15" : "border-border"}`}>
          ...
          <section>
            ...
            {readyItems.length > 0 && (
              <div className="mt-7 rounded-[1.75rem] border border-border bg-card p-5 shadow-soft sm:p-6">
                <div className="flex items-end justify-between gap-3"><div><p className="text-xs font-bold uppercase tracking-[0.16em] text-primary">Ready to order</p><h2 className="mt-1 text-xl font-bold">Choose an available option</h2></div><Badge variant="secondary">{readyItems.length} listed</Badge></div>
                <div className="mt-4 grid gap-3 sm:grid-cols-2">
                  {readyItems.map((item) => {
                    const unavailable = !item.made_to_order && item.stock <= 0;
                    const active = selectedReadyId === item.id;
                    return <button key={item.id} type="button" disabled={unavailable} onClick={() => chooseReady(item)} className={`flex gap-3 rounded-2xl border p-3 text-left transition-all disabled:cursor-not-allowed disabled:opacity-50 ${active ? "border-primary bg-secondary/35 ring-2 ring-primary/15" : "border-border hover:bg-secondary/20"}`}>
                      <img src={item.image_url} alt={item.name} className="size-20 rounded-xl object-cover" />
                      <span className="min-w-0"><span className="block font-bold">{item.name}</span><span className="mt-1 flex flex-wrap gap-x-2 gap-y-1 text-xs text-muted-foreground">{item.team && <span>{item.team}</span>}{item.size && <span>{item.size}</span>}</span><span className="mt-2 block font-bold text-primary">{formatPrice(item.price)}</span><span className="mt-1 block text-[11px] text-muted-foreground">{item.made_to_order ? "Made to order" : `${item.stock} ready now`}</span></span>
                    </button>;
                  })}
                </div>
            ...
            <div className="mt-5 rounded-[1.75rem] border border-border bg-card p-5 shadow-soft sm:p-6">
              <h2 className="text-lg font-bold">Want different colors, a different team, or something completely custom?</h2>
              <p className="mt-1 text-sm text-muted-foreground">Start a custom chat with JMB and request colors, size, theme, wording, or a design that is not already listed.</p>
              <Button variant="hero" className="mt-5 w-full" asChild><a href={customHref}><Sparkles aria-hidden /> Customize This / Start a Chat</a></Button>
            </div>
        ...
        <section className="mt-16 border-t border-border pt-12">
          ...
          <div className="mt-7 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-4">
            {designs.map((item, index) => <button key={item.id} type="button" onClick={() => { setSelectedReadyId(null); setSelectedIndex(index); window.scrollTo({ top: 0, behavior: "smooth" }); }} className="rounded-[1.5rem] border border-border bg-card p-3 text-left shadow-soft transition-transform hover:-translate-y-1"><img src={item.image} alt={item.name} className="aspect-[4/5] w-full rounded-[1.1rem] object-cover" loading="lazy" /><p className="px-2 pb-1 pt-3 text-sm font-bold">{item.name}</p></button>)}
          </div>
        </section>
